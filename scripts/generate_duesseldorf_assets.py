...
def main():
    ...
    url_geo = "https://raw.githubusercontent.com/codeforgermany/click_that_hood/main/public/data/duesseldorf.geojson"
    print(f"Downloading GeoJSON from {url_geo}...")
    with urllib.request.urlopen(url_geo) as response:
        geo_data = json.loads(response.read().decode())

    url_wiki = "https://de.wikipedia.org/wiki/Liste_der_Stadtteile_von_D%C3%BCsseldorf"
    ...
    for row in rows[1:]:
        ...
        if len(cols) < 4:
            continue
        st_name = clean_cell(cols[0])
        sb_num = clean_cell(cols[1])
    ...
    json_path = "src/data/duesseldorf_stadtteile.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(db_entries, f, ensure_ascii=False, indent=2)
    print(f"Successfully saved {len(db_entries)} entries to {json_path}")

    js_path = "src/data/duesseldorf_data.js"
    with open(js_path, "w", encoding="utf-8") as f:
        f.write("const DUESSELDORF_DATA = ")
        json.dump(db_entries, f, ensure_ascii=False, indent=2)
        f.write(";\nwindow.DUESSELDORF_DATA = DUESSELDORF_DATA;\n")
    print(f"Successfully saved {js_path}")

    svg_path = "src/data/duesseldorf_map.svg"
    joined_paths = "\n\t".join(svg_paths)
...
\t{joined_paths}
  ...
  <g id="map-labels-group" style="pointer-events: none;"></g>
    ...
    print("Düsseldorf assets generated successfully!")
