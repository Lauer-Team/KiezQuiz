...
def main():
    print("Starting data generation for Frankfurt Stadtteile...")
    os.makedirs("src/data", exist_ok=True)

    url_geo = "https://raw.githubusercontent.com/codeforgermany/click_that_hood/main/public/data/frankfurt-main.geojson"
    ...
    for row in rows[1:]:
        ...
        area = clean_cell(cols[2])
        pop = clean_cell(cols[3])
        if not re.search(r"\d", pop):
            pop = "0"
        if len(cols) >= 11:
            ob_name = normalize_ortsbezirk(clean_cell(cols[10]))
        else:
            ob_name = normalize_ortsbezirk(clean_cell(cols[4]))
    ...
    name_corrections = {
        "Sachsenhausen-N.": "Sachsenhausen-Nord",
        "Sachsenhausen-S.": "Sachsenhausen-Süd",
    }
    ...
    json_path = "src/data/frankfurt_stadtteile.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(db_entries, f, ensure_ascii=False, indent=2)
    print(f"Successfully saved {len(db_entries)} entries to {json_path}")

    js_path = "src/data/frankfurt_data.js"
    with open(js_path, "w", encoding="utf-8") as f:
        f.write("const FRANKFURT_DATA = ")
        json.dump(db_entries, f, ensure_ascii=False, indent=2)
        f.write(";\nwindow.FRANKFURT_DATA = FRANKFURT_DATA;\n")
    print(f"Successfully saved {js_path}")

    svg_path = "src/data/frankfurt_map.svg"
    joined_paths = "\n\t".join(svg_paths)
...
\t{joined_paths}
  ...
  <g id="map-labels-group" style="pointer-events: none;"></g>
    ...
    print("Frankfurt assets generated successfully!")
